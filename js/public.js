  ...
  async function fetchData() {
    const response = await fetch('https://cyber9.live/get-public-users', { method: 'POST' });
  ...
  async function fetchPromptDetails(title, name) {
    const response = await fetch(`https://cyber9.live/get-prompt-details?title=${encodeURIComponent(title)}&name=${encodeURIComponent(name)}`, { method: 'POST' });
    const data = await response.json();
